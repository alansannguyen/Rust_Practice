...
fn add(item: String, items: &mut Vec<String>) {
    items.push(item);
}

fn remove(index: String, items: &mut Vec<String>) {
    let int = index.parse::<usize>().unwrap();

    if int < items.len() {
        items.remove(int);
    }
    else {
        println!("Don't exist nerd");
    }
}

fn show(items: &mut Vec<String>) {
    let mut index = 0;
    for string in items {
...
fn main() {
    ...
    loop {
        ...
        match command_parts.get(0) {
            Some(&"add") => {
                if let Some(item) = command_parts.get(1) {
                    add(item.to_string(), &mut item_list);
                } else {
                    println!("Invalid 'add' command. Please do better.");
                }
            }
            Some(&"remove") => {
                if let Some(item) = command_parts.get(1) {
                    remove(item.to_string(), &mut item_list);
                } else {
                    println!("Invalid 'remove' command. Please do better.");
                }
            }
            Some(&"show") => {
                show(&mut item_list);
            ...
            Some(&"quit") => {
                println!("Leaving Nerd");
                return;
            }
            _ => {
                println!("Invalid command. Type Better.");
            }
        }
    }
}
